...
export default function SignupScreen({ navigation }) {
  ...
  async function handleContinue() {
    ...
    setLoading(true);
    // إرسال رمز تحقق فعلي عبر Supabase Auth (يحتاج مزود SMS مفعّل بلوحة Supabase)
    ...
    navigation.navigate('Otp', {
      phone,
      pendingProfile: { full_name: fullName, phone, email, university, specialization },
    });
  }
  ...
  return (
    <ScrollView contentContainerStyle={styles.wrap} keyboardShouldPersistTaps="handled">
      ...
      <Pressable style={styles.primaryBtn} onPress={handleContinue} disabled={loading}>
        {loading ? <ActivityIndicator color={colors.bg} /> : <Text style={styles.primaryBtnTxt}>متابعة</Text>}
      </Pressable>
    </ScrollView>
  );
}
